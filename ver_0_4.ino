...
void movePole(int pozice, byte naskladnit) {
  ...
  if(naskladnit == 1){   //funkce pro naskladnění
    ...
    //moveToZ(poziceZ[nZ]);
    moveToXZ(poziceX[nX], poziceZ[nZ]); //nova funkce, kombinujici pohyb po dvou osach zaroven
    moveToY(poziceY[2]);
    ...
    moveToXZ(poziceX[pozice], poziceZ[pozice]+nabiraciOffset); //nova funkce na simultanni pohyb dvou os
    ...
    moveToXZ(poziceX[pozice], poziceZ[pozice]); //nova funkce na simultanni pohyb dvou os
    ...
    moveToY(poziceY[0]); //cestovni pozice
    if(pozice == 6){  //v pripade vykladani z pozice vedle displaye si trochu popojed dolu
    ...
    moveToXZ(poziceX[vX], poziceZ[vZ]+20); //nova funkce na simultanni pohyb dvou os
...
void homeXZ() {
  ...
  int runn = 1;
  while ( runn ) //beží dokud nedoběhne na konec osy (nikdy) nebo dokud se neobrátí stav endstopu
  {
    if(osaX.currentPosition() != osaX.targetPosition() && digitalRead(X_MIN_PIN) == logEndstopX){
      osaX.run();
    }else{
      osaX.stop();
      runn = 2;
    ...
    if(osaZ.currentPosition() != osaZ.targetPosition() && digitalRead(Z_MIN_PIN) == logEndstopZ){
      osaZ.run();
    }else{
      osaZ.stop();
      if(runn == 2){
...
void moveToXZ(float vzdalenostX, float vzdalenostZ) {
   if (ishomeX && ishomeY && ishomeZ) {       //zjištění homu
#ifdef diag
    Serial.println(F("osy jsou v homu"));
#endif
  ...
  vzdalenostX = stoupaniX * vzdalenostX;
  vzdalenostZ = stoupaniZ * vzdalenostZ;
  osaX.moveTo(xMotorDir * vzdalenostX);
  osaZ.moveTo(zMotorDir * vzdalenostZ);
  if (osaX.targetPosition() / stoupaniX > delkaX  or osaZ.targetPosition() / stoupaniZ > delkaZ) { //pokud chce jet za hranice osy
#ifdef diag
    ...
    while (osaX.currentPosition() != osaX.targetPosition() or osaZ.currentPosition() != osaZ.targetPosition())
    {
      osaX.run();
      osaZ.run();
    }
